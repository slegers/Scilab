funcprot(0)
test_cases = list()
test_cases($+1) = struct('input', struct('x', 0), 'output', struct('solution', "dummy"))
...
function show(index)
    ...
    disp('Outputs')
    disp('solution')
    disp(tc.output.solution)
endfunction

function Result = check(index)
    tc = test_case(index)

    [solution] = solve(tc.input.x)

    Result = %t
    Result = Result & isequal(solution, tc.output.solution)
endfunction
